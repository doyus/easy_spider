...
from gxadmin.settings import BASE_DIR, STATIC_URL
...
def retrieve(request, model, filters: FuFilters = FuFilters()):
    ...
    return query_set
# def retrieve(request, model, filters: FuFilters = FuFilters()):
# # def retrieve(request, model, filters: FiltersCmsCrawlData = FiltersCmsCrawlData()):
#     """
#     根据提供的过滤条件从数据库中检索模型实例。
#     """
#     # 根据请求和过滤条件应用数据权限控制
...
#         filter_conditions = {}
#         print("aaaaaaa", query_params)
#         # 处理大于/小于查询
#         if 'created_atgt' in query_params:
#             print("找到了")
#             filter_conditions['created_at__gt'] = query_params.pop('created_at__gt')
#         if 'created_atgt' in query_params:
#             print("ccccc")
#             filter_conditions['created_at__lt'] = query_params.pop('created_at_lt')
#
#
#         # 添加剩余的普通查询条件
#         filter_conditions.update(query_params)
#
#         # 使用过滤条件查询模型实例
#         query_set = model.objects.filter(**filter_conditions)
#     else:
#         # 如果没有有效的过滤条件，则返回所有模型实例
#         query_set = model.objects.all()
#
