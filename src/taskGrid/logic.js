...
function saveService(type) {
    ...
    let text = LANG("确认要保存任务吗（不能用鼠标点击时，请按键盘回车键，执行一次即可）？", "Are you sure to save the task (if you can't use the mouse to click, please press the enter key)?");
    if (type == 1) { //任务另存为
        serviceId = -1;
        $("#create_time").val(formatDateTime(new Date()));
        text = LANG("确认要另存为任务吗（不能用鼠标点击时，请按键盘回车键，执行一次即可）？", "Are you sure to save the task as (if you can't use the mouse to click, please press the enter key)?");
    }
    // if (confirm(text)) {
